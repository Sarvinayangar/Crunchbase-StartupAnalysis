...
SELECT * FROM "USAInvestments2000";
SELECT COUNT(*) FROM "USAInvestments2000";

----Create new table for USA market after 2000 - 2014 and inserting revelant values from Investment table ----------------------------------------------------
CREATE TABLE IF NOT EXISTS "USAInvestments2000"(
"name" VARCHAR, 
market VARCHAR,
...
INSERT INTO "USAInvestments2000" ("name", market, funding_total_usd, status,
...
AND I.country_code = 'USA';

---Changing Data types and dropping unnecessary columns----------------------------------------------------------
ALTER TABLE "USAInvestments2000"
...
GROUP BY city
HAVING COUNT(*) >= 100 --Sample Population is 159 
ORDER BY city ASC;
...
GROUP BY market
HAVING COUNT(*) >= 100-- Sample population by market is 307
)
...
SELECT SUBSTRING((founded_quarter),2,LENGTH(founded_quarter)) FROM "USAInvestments2000"

WITH RECURSIVE Accumulate AS (
SELECT 
EXTRACT(YEAR FROM first_funding_at) AS "Year",
1 AS Num, 
seed AS "seed"
FROM "USAInvestments2000"
WHERE CAST(SUBSTRING(founded_quarter,2) AS INT) = 1
GROUP BY CAST(SUBSTRING(founded_quarter,2)
UNION ALL
SELECT
A."Year" AS "Year",
A.Num + 1 AS Num,
A."seed" + U.seed AS "seed"
FROM Accumulate A
JOIN "USAInvestments2000" U ON (A."Year" = EXTRACT(YEAR FROM U.first_funding_at))
WHERE A.num + 1 < 4
GROUP BY CAST(SUBSTRING(founded_quarter,2)
)
